...
. この時点で、モデルを RHOAI モデル サービングにデプロイする必要があります。
. もう 1 つのデータ接続を作成します...
.. ほぼ同じ情報を使用します
.. ただし、バケット名を `userX` から `models` に変更します
...
プロジェクトでモデル サーバーを作成します。`Multi-modelserving platform` を使用して、`Add model server` を選択します

* **Add model server** をクリックします
+
[.bordershadow]
...
* **追加** をクリックすると、モデル サーバーを作成できます。

== モデルのデプロイ

プロジェクトの **モデルとモデル サーバー** で、**モデルのデプロイ** を選択します。
...
* [**デプロイ**] をクリックします。
* モデルが正常にデプロイされると、15 ～ 30 秒後にステータスが緑色で表示されます。
+
[.bordershadow]
...
* これを行うには、[**推論エンドポイント**] 列の [**内部サービス**] リンクをクリックします。
...
- 実行中のワークベンチで、`insurance-claim-processing/lab-materials/04` フォルダーに移動します。
