...
まず、ArgoCD のインスタンスを名前空間にデプロイします。 +
これはアプリケーションのデプロイに使用されます。

- 次のテキストをコピーし、OpenShift ターミナルに貼り付けて ArgoCD をデプロイします。
+
[.lines_space]
[.console-input]
[source, text]
...
ArgoCD が名前空間にデプロイされたので、これを使用して GitOps 経由でアプリケーションをデプロイできます。

- 次のテキストの内容をコピーし、OpenShift ターミナルに貼り付けてアプリケーションをデプロイします。
+
[.lines_space]
[.console-input]
...
spec:
  ...
  source:
    ...
    repoURL: https://github.com/ritzshah/insurance-claim-processing-partners.git
    targetRevision: main
  syncPolicy:
    automated:
      prune: true
